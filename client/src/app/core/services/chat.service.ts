...
export class ChatService {
    private messagesSubject: BehaviorSubject<ReceivedMessage[]> =
        new BehaviorSubject<ReceivedMessage[]>([]);
    ...
    async sendMessage(senderId: number, content: string): Promise<void> {
        ...
        try {
            await lastValueFrom(
                this.http.post(`${this.apiUrl}/message/send`, message),
            );
    ...
    async loadMessages(chatRoomId: number): Promise<void> {
        try {
            ...
            const receivedMessages = await lastValueFrom(
                this.http.get<ReceivedMessage[]>(url),
            );
            ...
            console.log(
                `Loaded messages for chat room ${chatRoomId}:`,
                receivedMessages,
            );
        } catch (error) {
    ...
    async getChatRooms(): Promise<ChatRoom[]> {
        try {
            ...
            const chatRooms = await lastValueFrom(
                this.http.get<ChatRoom[]>(url),
            );
    ...
    async getChatRoomById(chatRoomId: number): Promise<ChatRoom | null> {
        try {
            ...
            const chatRoom =
                chatRooms.find((room) => room.id === chatRoomId) || null;
